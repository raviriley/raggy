...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`🔍 [GRAPH IMPORT] Processing ${transactions.length} transactions and ${relationships.length} relationships`);
    ...
    try {
      await session.run('CREATE CONSTRAINT transaction_hash_constraint IF NOT EXISTS FOR (t:Transaction) REQUIRE t.hash IS UNIQUE');
    ...
    for (let i = 0; i < transactionCount; i += batchSize) {
      ...
      console.log(`✅ [GRAPH IMPORT] Created ${processedTx}/${transactionCount} transaction nodes`);
    }
    
    // 4. Create relationships between transactions (if any)
    if (relationships.length > 0) {
      console.log(`🔗 [GRAPH IMPORT] Creating ${relationships.length} direct transaction relationships`);
      ...
      for (let i = 0; i < relationships.length; i += batchSize) {
        ...
        console.log(`✅ [GRAPH IMPORT] Created ${processedRel}/${relationships.length} transaction relationships`);
      }
    }
    ...
    const createEmbeddings = url.searchParams.get('createEmbeddings') === 'true';
    
    if (createEmbeddings) {
      console.log('📝 [GRAPH IMPORT] Preparing transaction descriptions for embedding');
    ...
    await session.close();
    return NextResponse.json(
      { error: 'Failed to import graph data', details: (error instanceof Error ? error.message : String(error)) },
      { status: 500 }
    );
  }
} 