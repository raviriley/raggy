...
export async function POST(request: NextRequest) {
  try {
    ...
    if (result.length === 0) {
      return NextResponse.json(
        { error: 'Source or target node not found', query: cypher, params: { sourceId: sourceIdInt, targetId: targetIdInt } },
        { status: 404 }
      );
    }
    ...
    return NextResponse.json(
      { error: 'Failed to create relationship', details: (error as Error).message },
      { status: 500 }
    );
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to fetch relationships', details: (error as Error).message },
      { status: 500 }
    );
  }
} 