  ...
  try {
    ...
    const extractPatterns = (tx: TransactionData) => {
      ...
      const patterns: Record<string, string[]> = {
        ...
        otherHashes: (text.match(/[a-fA-F0-9]{32,}/gi) || [])
    ...
    function extractTokens(text: string): string[] {
      const commonTokens = [
        'eth', 'btc', 'usdc', 'usdt', 'dai', 'link', 'aave', 'uni', 'sushi',
        'matic', 'weth', 'wbtc', 'sol', 'avax', 'bnb', 'ftm', 'op', 'arb'
      ];
    ...
    function extractActions(text: string): string[] {
      const actionKeywords = [
        'transfer', 'swap', 'deposit', 'withdraw', 'mint', 'burn', 'stake',
        'unstake', 'borrow', 'repay', 'claim', 'bridge', 'approve', 'exchange',
        'provide', 'remove', 'liquidate', 'lend'
      ];
    ...
    function extractChains(text: string): string[] {
      const chainKeywords = [
        ...
        'binance', 'bnb', 'bsc', 'avalanche', 'avax', 'fantom', 'ftm', 'solana', 'sol',
        'mainnet', 'testnet', 'goerli', 'sepolia', 'mumbai', 'rinkeby'
      ];
    ...
    function extractProtocols(text: string): string[] {
      const protocolKeywords = [
        'uniswap', 'sushiswap', 'aave', 'compound', 'maker', 'curve',
        'balancer', 'yearn', 'synthetix', '1inch', 'pancakeswap', 'opensea',
        'lido', 'convex', 'gmx', 'trader joe', 'dydx', 'metamask', 'chainlink'
      ];
      
      const protocolMatches = new Set<string>();
      
      for (const protocol of protocolKeywords) {
        const regex = new RegExp(`\\b${protocol.replace(' ', '\\s+')}\\b`, 'gi');
        const matches = text.match(regex) || [];
    ...
    const txPairCount = new Map<string, {count: number, patterns: Set<string>, types: Set<string>}>();
    
    for (const rel of relationships) {
      const pairKey = [rel.source, rel.target].sort().join('_');
      
      if (!txPairCount.has(pairKey)) {
        txPairCount.set(pairKey, {count: 0, patterns: new Set(), types: new Set()});
      }
    ...
    return NextResponse.json(
      { error: 'Failed to process transactions', details: (error as Error).message },
      { status: 500 }
    );
  }
} 