...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log(`✅ [GRAPH] Found ${relationships.length} relationships for transaction ${hash}`);
    ...
    for (const record of relationshipsResult.records) {
      ...
      const relationshipCount = neighborRelationshipsResult.records[0].get('relationshipCount').toNumber();
    ...
    await session.close();
    return NextResponse.json(
      { error: 'Failed to fetch transaction neighbors', details: (error instanceof Error ? error.message : String(error)) },
      { status: 500 }
    );
  }
} 