...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`📝 [LETTA QUERY] Processing message: "${message.substring(0, 50)}${message.length > 50 ? '...' : ''}"`);
    ...
    if (!agentId) {
      try {
        ...
        return NextResponse.json(
          { 
            error: 'No agent ID found. Please create and store an agent ID first.',
    ...
    console.log(`🔌 [LETTA QUERY] Using streaming endpoint: ${streamingEndpoint}`);
    ...
    if (!response.ok) {
      ...
      try {
        ...
        console.log(`❌ [LETTA QUERY] API request failed with status ${response.status}`);
      }
      ...
      async start(controller) {
        ...
        try {
          while (true) {
            ...
            if (done) {
              console.log(`✅ [LETTA QUERY] Stream completed after ${chunkCount} chunks and ${eventCount} events`);
              controller.close();
              break;
            }
            ...
            console.log(`📦 [LETTA QUERY] Received chunk #${chunkCount} (${value.length} bytes)`);
              ...
              if (line.startsWith('data: ')) {
                ...
                try {
                  ...
                  if (jsonData.messages) {
                    for (const msg of jsonData.messages) {
                      if (msg.message_type === 'assistant_message' && msg.content) {
                        ...
                        const newContent = msg.content.slice(accumulatedAssistantMessage.length);
                        accumulatedAssistantMessage = msg.content;
                        
                        if (newContent.length > 0) {
                          console.log(`💬 [LETTA QUERY] Assistant message update: "${newContent.substring(0, 30)}${newContent.length > 30 ? '...' : ''}"`);
                        ...
                        console.log(`🧠 [LETTA QUERY] Agent reasoning: "${msg.reasoning?.substring(0, 30)}${msg.reasoning?.length > 30 ? '...' : ''}"`);
                      }
                    }
                  }
                  ...
                  if (jsonData.delta && jsonData.delta.content) {
                    console.log(`🔤 [LETTA QUERY] Token delta: "${jsonData.delta.content}"`);
                    controller.enqueue(new TextEncoder().encode(jsonData.delta.content));
                  }
                } catch (e) {
